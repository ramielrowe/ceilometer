...
class EventTest(EventTestBase):
    ...
    def test_save_event_with_notification(self):
        now = datetime.datetime.utcnow()
        event = models.Event(TEST_NOTICE[u'message_id'],
                             TEST_NOTICE[u'event_type'],
                             now, [], notification=TEST_NOTICE)
        problem_events = self.conn.record_events([event])
        self.assertEquals(0, len(problem_events))

    def test_save_event_with_duplicate_notification(self):
        now = datetime.datetime.utcnow()
        event = models.Event(TEST_NOTICE[u'message_id'],
                             TEST_NOTICE[u'event_type'],
                             now, [], notification=TEST_NOTICE)
        problem_events_1 = self.conn.record_events([event])
        problem_events_2 = self.conn.record_events([event])
        self.assertEquals(0, len(problem_events_1))
        self.assertEquals(1, len(problem_events_2))
        self.assertEquals(models.Event.DUPLICATE, problem_events_2[0][0])
...
class GetEventTest(EventTestBase):
    ...
    def test_get_event_notification(self):
        event_filter = storage.EventFilter(self.start, self.end)
        events = self.conn.get_events(event_filter)
        notif = events[0].notification
        self.assertIsNotNone(notif)
        self.assertEqual(TEST_NOTICE[u'payload'][u'instance_id'],
                         notif['payload']['instance_id'])
